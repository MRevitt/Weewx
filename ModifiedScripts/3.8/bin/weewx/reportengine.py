...
class AWS_Sync(ReportGenerator):
    ...
    def run(self):
        ...
        if 'DRY_RUN' in self.skin_dict:
            command = subprocess.Popen([aws, s3_cmd, s3_sync, '--dryrun', local_root, s3_root, cache_control, cache_age ], stdout = LogFile )
        else:
            command = subprocess.Popen([aws, s3_cmd, s3_sync,             local_root, s3_root, cache_control, cache_age ], stdout = LogFile )
